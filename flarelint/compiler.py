...
r"""The FlareLint Language for Rules (FLR) has this syntax:

rule = comment-section? type-section when-section+ test-section+ message-section

type-section = 'rule:' ('error' | 'warning')

extension-section = 'extensions:' name+

when-section = 'when:' condition
...
predicate = 'snippet' quote | 'fail' | pov tag | pov '-style' tag |
pov '-empty' tag | pov '-padding' tag | pov '-position' tag number
| pov '-contains' tag quote | pov '-text' tag quote | pov '-ends' tag quote | 
pov "-starts" tag quote | pov '-attribute' tag name | 
pov '-attribute-contains' tag name quote | 
pov '-attribute-ends' tag name quote | 
pov '-snippet' quote | pov '-variable' name | first | last

pov = 'self' | 'child' | 'parent' | 'after' | 'before' | 'previous' |
...
- Sections must start at the end of a line.

- Items enclosed in single quotes (') are literal. 

- The question mark (?) means "zero or one"

- The plus sign (+) means "one or more"

- The asterisk sign (*) means "zero or more"

- The vertical bar (|) means "or"

- Items enclosed in parentheses are grouped together. So (x y) | z is
  not the same as x (y | z).
